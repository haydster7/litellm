...
class TestTeamModelUpdate:
    ...
    async def test_team_move_releases_the_public_name_from_the_source_team(self, moved_row, source_backing, released):
        ...
        router = MagicMock(**{"get_model_ids.return_value": ["m-moved"], "get_model_list.return_value": []})
...
class TestDeleteTeamBYOKModelGhost:
    ...
    async def test_delete_strips_public_name_and_refreshes_cache(self):
        ...
        with (
            ...
            patch(f"{_PS}.store_model_in_db", True),
            patch(f"{_PS}.premium_user", True),
            patch(f"{_PS}.llm_router", MagicMock(**{"get_model_list.return_value": []})),
            patch(f"{_PS}.proxy_logging_obj", MagicMock()),
    ...
    async def test_delete_legacy_team_model_scrubs_stale_alias_and_keeps_gateway_access(
        ...
        mock_router.get_model_list.return_value = [{"model_name": public_name}]
