...
async fn validate_environment(
    ...
    let token = super::resolve_entra(config, env_lookup).await?.ok_or_else(|| {
        Error::Auth(
            "Missing Azure Document Intelligence credentials - set AZURE_DOCUMENT_INTELLIGENCE_API_KEY or configure Entra ID".into(),
        )
    })?;
    Ok(
