...
#[tokio::test]
async fn existing_file_id_bypasses_upload_and_maps_v3_params() {
    ...
    request.document = request.document.with_source("reducto://already.pdf".into());
...
#[tokio::test]
async fn legacy_wraps_enhance_under_options() {
    let (base, seen, server) = mock_server(vec![MockResponse::json(json!({"chunks":[]}))]).await;
    let mut request = wire_request(
        "reducto/parse-legacy",
        &base,
        json!({"enhance":{"agentic":[{"type":"table"}]}}),
    );
    request.document = request.document.with_source("reducto://legacy.pdf".into());

    perform_ocr(request).await.unwrap();
    server.await.unwrap();
    assert_eq!(
        request_body(&seen.lock().unwrap()[0]),
        json!({
